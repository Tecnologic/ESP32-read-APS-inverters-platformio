...
// *                              SETUP
// *****************************************************************************
// RXD2 / TXD2 / ZB_SERIAL are defined centrally per-board in
// board_pins_wemos_d1_mini32.h resp. board_pins_esp32c3_supermini.h
// (included via AAA_INCLUDES.h, selected by the BOARD_ESP32C3_SUPERMINI
// build flag)
void setup() {
  Serial.begin(115200);

  ZB_SERIAL.begin(115200, SERIAL_8N1, RXD2, TXD2);
  Serial.printf("Zigbee Serial Txd is on pin: %d\n", TXD2);
  Serial.printf("Zigbee Serial Rxd is on pin: %d\n", RXD2);
 ...
 // digitalWrite(ZB_TX, LOW); //
  ...
  pinMode(EXTRA_OUT_PIN, OUTPUT); //
  digitalWrite(EXTRA_OUT_PIN, LOW); //
